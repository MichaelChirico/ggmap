...
get_stamenmap <- function(
  ...
  .Defunct(msg = "Stamen map tiles are now hosted by Stadia Maps, use `get_stadiamap()`.")
  get_stamenmap_tile <- NULL; rm(get_stamenmap_tile) # cran guard for removed function
...
# removed 11/01/2023 with moving get_stamenmap() to defunct
